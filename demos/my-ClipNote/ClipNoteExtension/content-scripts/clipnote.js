(function () {
  ...
  function updete_video() {
    ...
    if (videoObj.length > 0) {
      // 检测视频列表是否发生变化
      if (
        videoObj.length !== _videoObj.length ||
        videoSrc.toString() !== _videoSrc.toString()
      ) {
        _videoSrc = videoSrc;
        _videoObj = videoObj;
        ...
        _videoObj.forEach(function (videoElement) {
          ...
          videoElement.addEventListener("timeupdate", function () {
            if (cnt) {
              ...
              if (cnt.includes("-")) {
                if (
                  video.currentTime >= cnt.split("-")[1] &&
                  parseInt(video.currentTime) == parseInt(cnt.split("-")[1])
                ) {
                  video.pause();
                  cnt = "";
                }
    ...
    if (elements.length > 0 && elements[0].getAttribute("title")) {
      for (var i = 0; i < elements.length; i++) {
        if (elements[i].getAttribute("title") == cnf) {
          var previousSibling =
            elements[i].parentNode.parentNode.previousElementSibling;
          setTimeout(function () {
  ...
  function createWebSocket() {
    ...
    socket.addEventListener("message", function (event) {
      ...
      if (json["action"] == "seek") {
        ...
        var domain = url.host;
        if (
          (href().includes(nocnf(json["url"])) ||
            nocnf(json["url"]).includes(href())) &&
          domain !=
            href()
              .replace("https://", "")
              .replace("http://", "")
      ...
      if (json["action"] == "snapshots") {
        ...
        try {
          ...
          if (previewPage && previewPage.document) {
            ...
            alert(
              "无法打开窗口，请设置浏览器权限（设置->cookie和网站权限->弹出窗口和重定向->允许）"
            );
          }
          return;
        }
